...
# /****                  PROGRAM DESCRIPTION                                 *
# Check for tea program unit to have received a payment for future month ,
...
# *  OUTPUT FILES:                  NONE                                      *
# *																			  *
# *  ERROR FILES,LOG FILES         tea_task_created, tea_task_creation_error  *
# *                                                                           *
# *  RECOVERY  INSTRUCTIONS        Re run the program                                                                        *
# *                                                                           *
# *****************************************************************************

task :tea_monthly_payments_from_batch => :environment do
	batch_user = User.where("uid = '555'").first
    AuditModule.set_current_user=(batch_user)
	filename = "batch_results/monthly/tea_monthly_payments/results/tea_task_created"+ "#{Date.today.strftime("%m%d%Y")}"+"#{Time.now.strftime("%H%M%S")}" + ".txt"
	error_filename = "batch_results/monthly/tea_monthly_payments/results/tea_task_creation_error"+ "#{Date.today.strftime("%m%d%Y")}"+"#{Time.now.strftime("%H%M%S")}" + ".txt"
	...
	# # Create new file called 'filename.txt' at a specified path.
	    ...
	    step1 = ProgramUnit.joins("INNER JOIN program_unit_members
	                     ...
	                     ON (PROGRAM_WIZARDS.PROGRAM_UNIT_ID=PROGRAM_UNITS.ID
	                         AND ( PROGRAM_WIZARDS.RUN_ID = ( SELECT RUN_ID
	                                                          ...
	                                                          and A.run_month <= date(date_trunc('month',current_date)+'1month')
	                    ...
	                    inner join (select program_unit_id
	                                      ...
	                                      union
	                                         ...
	                                         (Extract(month from action_date) >= Extract(month from current_date)
	                                         and  Extract(year from action_date) >=  Extract(year from current_date))
		...
		if program_unit_collection.present?
			program_unit_collection.each do |each_program_unit|
				...
				in_state_payments = InStatePayment.get_payment_from_program_unit_id_and_payment_month(each_program_unit.id,current_month)
				...
				# already payment is done for previous month
                    ...
                    #6175- "TEA",5760-"Regular",6172- "Participant",6191 - "Submitted",6201- "Authorized"
					payment_line_item_object = PaymentLineItem.set_payment_line_item_object(ls_program_unit_id,current_month,6175,5760,ls_client_id,6172,ld_program_benefit_amount,6191,ls_run_id,6201)
...
end  #:tea_monthly_payments_from_batch => :environment do
