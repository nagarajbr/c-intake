...
task :generate_daily_payment_file_to_ebt_for_payment => :environment do
...
filename = "batch_results/daily/EBT/EBT_payments/outbound/ebt_daily_pay_" + ls_file_date.to_s + ".txt"
log_filename = "batch_results/daily/EBT/EBT_payments/results/ebt_daily_pay_log_" + "#{Date.today.strftime("%m%d%Y")}"+"#{Time.now.strftime("%H%M%S")}" + ".txt"
...
arg_new_status = 6193 # payment issued
arg_line_item_type_workp = 6176 # Work pays
arg_line_item_type_tea = 6175 # TEA
payments = PaymentLineItem.get_daily_ebt_payments(arg_payment_status,arg_auth_status,arg_line_item_type_workp, arg_line_item_type_tea)
...
ebt_payment_record = ebt_payment_record + 'CASH DAILY      '
...
if payments.present?
	payments.each do |payment_line_item|
		...
		# if payment_line_item.payment_type == 5760 && payment_line_item.line_item_type = 6175
		...
		if payment_line_item.payment_type != 6228 #supplement
		...
		if l_account_number_collection.present?
			l_account_object = l_account_number_collection.first
		else
			ls_error = 'Y'
			ls_error_cnt = ls_error_cnt + 1
		...
		ebt_payment_record = ebt_payment_record + l_account_object.account_number.to_s.rjust(12,'0')
		...
		l_run_id = payment_line_item.determination_id
		l_determination_id = payment_line_item.determination_id
		ls_update_ind = 'N'
		if payment_line_item.payment_type == 5760 || payment_line_item.payment_type == 6229 #Regular or retro
			if payment_line_item.payment_type == 6229 #Retro
				last_pgm_wiz = ProgramWizard.get_latest_run_from_program_unit_id(payment_line_item.reference_id)
				if last_pgm_wiz.present?
					l_run_id = last_pgm_wiz.run_id
				else
					ls_error = 'Y'
					ls_error_cnt = ls_error_cnt + 1
					log_file.puts("No program benefit wizard found for payment id: " + payment_line_item.id.to_s)
					next
				end
			end
			active_adults_list = ProgramBenefitMember.get_active_adult_client_ids_associated_with_run_id(l_run_id)
			if active_adults_list.present?
				active_adults_list.each do |active_members|
		   			...
		   			if payment_line_item.line_item_type = 6175
		   				...
		   				arg_work_participation_status = ClientCharacteristic.get_current_work_characteristics(arg_client_id)
		   			else
		   			...
		   			if payment_line_item.payment_type == 5760
			   			arg_month = 1
			   			program_benefit_detail_collection = ProgramBenefitDetail.get_program_benefit_detail_collection(l_run_id, arg_month)
	   			...
	   			puts ("No active adults for : " + payment_line_item.id.to_s)
	   			if arg_service_pgm_id == 4 && (payment_line_item.payment_type == 5760 || payment_line_item.payment_type == 6229)
					ls_error = 'Y'
					ls_error_cnt = ls_error_cnt + 1
		...
		if ls_error == 'N'
			if ebt_payment_record.length != 80
				ls_error_cnt = ls_error_cnt + 1
				log_file.puts("Incorrect record length: " + ebt_payment_record.length.to_s + " " + ebt_payment_record)
			else
				file.puts(ebt_payment_record)
