...
has_paper_trail :class_name => 'AlienVersion',:on => [:update, :destroy]
     ...
     # attr_accessor :non_citizen_type, :country_of_origin,:refugee_status,:alien_DOE,:alien_no,:client_citizenship, :sves_type
      attr_accessor :client_citizenship, :sves_type
  ...
  def self.human_attribute_name(attr, options = {})
   ...
   # validates_presence_of :client_citizenship, message: "is required "
   validate :client_citizenship_required,:valid_if_citizenship_is_No, :valid_alien_DOE?


  def client_citizenship_required
    if client_citizenship.blank?
       errors.add(:client_citizenship, "is required.")
    end
  end
	...
	def valid_if_citizenship_is_No
	      ...
	      if client_citizenship == "N" && client_citizenship.present?
	       		...
	       		if refugee_status.blank? then
	       			 errors.add(:refugee_status, "is required.")
	       		end
	       		if alien_DOE.blank? then
	       			 errors.add(:alien_DOE, "is required.")
	       		end
            if alien_DOE.present? && alien_DOE.to_date > Date.today then
